...
// Per-shortcut AI brief (home Intelligence panel) — auto-loaded on expand.
// Pre-generated by the cron, so this is normally an instant cache hit. On
// failure the block stays hidden and the model picker is unaffected.
async function loadShortcutItemBrief(sc) {
  const el = sc.querySelector('.ti-shortcut-ai');
  const id = sc.dataset.id;
  if (!el || !id || el.dataset.state) return; // loaded / loading / unavailable
  el.dataset.state = 'loading';
  el.hidden = false;
  const head = `<div class="ai-result-head"><span class="ai-result-label">AI Brief</span><span class="ai-result-badge">AI</span></div>`;
  el.innerHTML = `<div class="ai-result">${head}<div class="ai-result-body ai-result-loading">Generating…</div></div>`;
  try {
    const res = await fetch('/api/insight', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ type: 'shortcut', id }) });
    const data = res.ok ? await res.json() : null;
    if (!data || !data.content) { el.hidden = true; el.innerHTML = ''; el.dataset.state = 'unavailable'; return; }
    el.innerHTML = `<div class="ai-result">${head}${renderBriefBody(data.content, data.sources)}<button type="button" class="ai-result-deeper">Open in chat ↗</button></div>`;
    el.querySelector('.ai-result-deeper')?.addEventListener('click', (e) => {
      e.stopPropagation();
      window.dispatchEvent(new CustomEvent('open-prompt-modal', {
        detail: { basePrompt: sc.dataset.prompt || '', name: sc.dataset.name || 'Shortcut', iconKey: sc.dataset.iconKey || '', count: 1 },
      }));
    });
    el.dataset.state = 'done';
  } catch (_) { el.hidden = true; el.innerHTML = ''; el.dataset.state = 'unavailable'; }
}
...
function renderShortcutsSidebar(container, route, isHome, isCustom = false, customTerm = '') {
      ...
      html += renderTIAccordion({
        ...
        bodyHTML: `
          ...
          <ul class="ti-item-list ti-item-list-shortcuts" data-group="${escapeAttr(g.key)}">
            ${items.map(s => tiShortcutItem(s, topicName, g.key, isHome)).join('')}
          </ul>
        `,
      });
    });
  ...
  container.querySelectorAll('.ti-shortcut-trigger').forEach(trig => {
    trig.addEventListener('click', (e) => {
      ...
      if (willOpen) loadShortcutItemBrief(sc); // no-op without a .ti-shortcut-ai slot (non-home)
...
function tiShortcutItem(shortcut, topicName, groupKey, withBrief = false) {
  const prompt = shortcut.prompt.replace(/\{topic\}/gi, topicName);
    ...
    <li class="ti-item-row">
      <div class="ti-shortcut" data-prompt="${escapeAttr(prompt)}" data-name="${escapeAttr(shortcut.name)}" data-icon-key="${escapeAttr(shortcut.icon)}" data-group="${escapeAttr(groupKey || '')}" data-id="${escapeAttr(shortcut.id || '')}">
        ...
        <div class="ti-shortcut-panel"><div class="ti-shortcut-panel-inner">
          ${withBrief ? '<div class="ti-shortcut-ai" hidden></div>' : ''}
