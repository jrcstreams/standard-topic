...
const NI_X_SVG = '<svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>';
...
async function renderNewsBriefInto(panel, card, attempt = 0) {
  ...
  try {
    ...
    if (data && data.content) {
      ...
      const drawers = drawerLinkHTML('Search story', '#/custom/' + encodeURIComponent(card.dataset.title || ''), DRAWER_SEARCH_IC, '', { noArrow: true })
...
export function newsCardHTML(item) {
  ...
  return `
    <article class="news-card" data-title="${escapeAttr(title)}" data-desc="${escapeAttr(descFull.slice(0, 500))}" data-url="${escapeAttr(url)}" data-date="${escapeAttr(pubDate)}">
      <!-- revamp1447: the meta line LEADS the card — source pill · time on the
           left, share on the right — and the title sits under it. Same DOM
           nodes as before, reordered, so the share panel and the open-state
           rules keep working. -->
      <div class="news-card-foot news-card-foot--lead">
    ...
    <div class="newsfeed-head section-card-head newsfeed-head--home${tabbed ? ' newsfeed-head--latest' : ''}">
      <div class="newsfeed-headrow">
        <div class="newsfeed-headtext">
          <h3 class="newsfeed-title section-card-title"><span class="newsfeed-title-ic" aria-hidden="true"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3.5" y="4.5" width="17" height="15" rx="2.6"/><path d="M7.2 9h5.2" stroke-width="2.6"/><path d="M7.2 13h9.6M7.2 16.2h6.4"/></svg></span><span class="newsfeed-title-main">${feedTitle}</span></h3>
        </div>
