...
async function generateInsight(sql, input) {
  ...
  if (type === 'shortcut' && input.builder) {
    ...
    try { out = await generate(p, { grounded: useGrounding, model: genModel, maxTokens: 2048, thinkingBudget: useGrounding ? BUILDER_THINKING : 0 }); } catch (_) { out = null; }
    ...
    const secResults = await Promise.all(entity.sections.map(async (s, i) => {
      ...
      let o = null;
      try { o = await generate(p, { grounded: ground, model: genModel, maxTokens: 600, thinkingBudget: ground ? GROUNDED_THINKING : 0 }); } catch (_) {}
    ...
    wasGrounded = useGrounding;
    try { out = await generate(prompt, { grounded: useGrounding, model: genModel, maxTokens, thinkingBudget: useGrounding ? GROUNDED_THINKING : 0 }); } catch (_) { out = null; }
