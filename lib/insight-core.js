...
function builderPrompt(scope, group, grounded, headlines) {
  ...
  // The feed headlines skew toward whatever's loud this week (often one company),
  // so feed them GROUP-AWARELY: Get Caught Up treats them as primary source
  // material; Deep Dive gets them only as light context (go beneath them); Analysis
  // and 101 Resources get none — they reason broadly / evergreen and grounding
  // supplies any current facts, so the feed would only bias + narrow them.
...
async function generateInsight(sql, input) {
  ...
  if (type === 'shortcut' && input.builder) {
    ...
    const tb = input.think !== undefined ? Number(input.think) || 0 : 0;   // debug knob
    try { out = await generate(p, { grounded: true, model: genModel, maxTokens: 1700, thinkingBudget: tb }); } catch (_) { out = null; }
  ...
  const _debug = (input.debug && out && out.meta) ? {
    grounded: wasGrounded,
    searches: out.searches || 0,
    webSearchQueries: (out.meta.webSearchQueries || []).slice(0, 10),
    groundingChunks: ((out.meta.groundingChunks) || []).length,
    groundingSupports: ((out.meta.groundingSupports) || []).length,
    citationsKept: Array.isArray(sources) ? sources.length : Object.keys(sources || {}).length,
    finish: out.finish,
    metaKeys: Object.keys(out.meta || {}),
    chunkSample: JSON.stringify(((out.meta.groundingChunks) || []).slice(0, 2)),
    rawMetaHead: JSON.stringify(out.meta).slice(0, 700),
  } : undefined;
  return { content, summary, sources, headlines: headlinesOut, cached: false, generatedAt: new Date().toISOString(), ...(_debug ? { _debug } : {}) };
}
