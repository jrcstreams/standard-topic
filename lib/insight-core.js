...
async function generateInsight(sql, input) {
  ...
  if (type === 'shortcut' && input.builder) {
    ...
    try { out = await generate(p, { grounded: useGrounding, model: genModel, maxTokens: 1700 }); } catch (_) { out = null; }
    if (useGrounding && (!out || !out.text)) {
      wasGrounded = false;
      try { out = await generate(p, { grounded: false, model: genModel, maxTokens: 1700 }); } catch (_) { out = null; }
    }
