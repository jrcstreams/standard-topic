...
function newsPrompt(e, grounded = true, related = []) {
  ...
  return [
    ...
    (!grounded && canTimeline)
      ? `TIMELINE RULE — build the Timeline ONLY from the dated coverage listed below and from dates stated in the story itself. Every entry must be an event one of those items actually reports. Do NOT add events from your own knowledge, and do NOT estimate or infer a date. If that leaves fewer than two entries, omit the Timeline section entirely rather than filling it.`
      : '',
    ``,
    `Story: "${e.title}"`,
