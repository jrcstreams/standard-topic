...
async function trendStories(sql, query) {
  ...
  try {
    const h = await sql.query(
      `SELECT title, url FROM news_stories WHERE search_vector @@ websearch_to_tsquery('english', $1)
...
async function generateInsight(sql, input) {
  ...
  const overviewHeads = type === 'shortcut' ? await overviewHeadlines(sql, entity.topic, entity.group) : [];
  const trendRelated = type === 'trend' ? await trendStories(sql, entity.query) : [];
  const headlinesOut = (type === 'trend' ? trendRelated : overviewHeads)
  ...
  if (type === 'shortcut') {
    ...
    else if (!sources.length && overviewHeads.length) {
