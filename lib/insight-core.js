  ...
  try {
    const rows = topic.slug === 'home'
      ? await sql.query(
          ...
          `WITH ranked AS (
             SELECT ns.title, ns.url, ns.source_name, ns.published_at,
           ...
           SELECT title, url, source_name, published_at FROM ranked
            WHERE rn <= $2
...
async function generateInsight(sql, input, opts = {}) {
  ...
  // it was.
  const builderHeads = isBuilder ? await builderHeadlines(sql, entity.topic, entity.group === 'daily' ? 28 : 14) : [];
