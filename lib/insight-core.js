...
const SECTION_FILLER = new Set(['watch', 'tracker', 'roundup', 'update', 'updates', 'snapshot', 'pulse', 'spotlight', 'report', 'reports', 'briefing', 'overview', 'news', 'the', 'and', 'for', 'this', 'that', 'with', 'from', 'your', 'today', 'daily', 'weekly', 'latest', 'check', 'deep', 'dive', 'guide', 'explained', 'digest', 'recap', 'insights', 'insight', 'analysis', 'global', 'current', 'live', 'now']);
...
async function overviewHeadlines(sql, scope, group, sections) {
  ...
  try {
    const rows = topic.slug === 'home'
      ? await sql.query(
        `SELECT s.name AS section, ns.title, ns.url
           FROM unnest($1::text[], $2::text[]) WITH ORDINALITY AS s(name, q, ord)
           CROSS JOIN LATERAL (
             SELECT title, url FROM news_stories
      ...
      : await sql.query(
        `SELECT s.name AS section, ns.title, ns.url
           ...
           CROSS JOIN LATERAL (
             SELECT ns.title, ns.url FROM news_stories ns
    ...
    for (const r of rows) {
      if (!r.url) continue;
      (out[r.section] || (out[r.section] = [])).push({ title: r.title || '', url: r.url });
    }
  } catch (_) { return {}; }
  return out;
}
...
async function generateInsight(sql, input) {
  ...
  // Display payload: a per-section { name: [{title,url}] } MAP for overviews (so
  // each insight shows its own stories), a flat list for trends.
  const headlinesOut = type === 'trend'
    ? trendRelated.map((h) => ({ title: h.title || '', url: h.url || '' })).filter((h) => h.url)
    : overviewHeadsMap;
