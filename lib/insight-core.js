...
async function gatherTrendFacts(sql, query) {
  let stories = [];
  ...
  // SerpAPI results LEAD (freshest live coverage), then our DB facts, deduped by
  // URL — so item #1 the model sees is the most current article.
  const seen = new Set();
  const facts = [];
  for (const f of [...serp, ...dbFacts]) {
    if (f.url && !seen.has(f.url)) { seen.add(f.url); facts.push(f); }
  }
  return { facts: facts.slice(0, 8), fromSerp };
}
...
function trendRagPrompt(query, ctx, facts) {
  ...
  const lines = [
    ...
    `SUMMARY: one sentence (max ~25 words) stating why "${query}" is trending right now, anchored to the SINGLE most recent development in the items below. State the OUTCOME (score, result, decision, consequence) when the event has concluded — never "just played", "faced off", or "is set to" if the items show the result. The SUMMARY must convey the same key fact as the DETAIL.`,
...
async function generateInsight(sql, input) {
  ...
  if (type === 'shortcut' && input.builder) {
    ...
    const gathered = await gatherTrendFacts(sql, entity.query);
