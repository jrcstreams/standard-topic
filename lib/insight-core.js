...
function sectionPrompt(scope, section, grounded, headlines) {
  ...
  const headBlock = (headlines && headlines.length)
    ? `\n\nCURRENT HEADLINES from our news feed (newest first) — treat these as primary, current source material; build from these concrete stories and reference them by name, preferring them over memory:\n${headlines.map((h, i) => `${i + 1}. ${h.title}${h.url ? ` — ${h.url}` : ''}`).join('\n')}`
    : '';
  return [
    ...
    `Open with a 2-3 sentence summary that LEADS with the single most important and most RECENT development relevant to this insight right now. Then add 2-3 concise takeaway bullet points, each on its own line starting with "- ". No section headers or other markdown.`,
    SPECIFICITY_RULES,
    grounded
      ? `Use Google Search NOW for the most current facts as of today, and rely on those results over your own prior knowledge — every date, name, and event must be current, not from an earlier year. Cite the sources you used.`
      : '',
  ].filter(Boolean).join('\n') + headBlock;
}
