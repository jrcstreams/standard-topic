...
const GROUNDED_THINKING = parseInt(process.env.AI_THINKING_BUDGET || '512', 10);
// Builders synthesize 3-5 in-depth sections — a little more room to think.
const BUILDER_THINKING = parseInt(process.env.AI_BUILDER_THINKING_BUDGET || '512', 10);
...
const GROUND_EMPTY_TRIP = parseInt(process.env.AI_GROUND_EMPTY_TRIP || '3', 10);
async function groundingTripped(sql) {
  const n = await surfaceCount(sql, 'ground:empty');
...
async function generateInsight(sql, input, opts = {}) {
  ...
  if (type === 'shortcut' && input.builder) {
    ...
    let lastDiag = null;
    try { out = await generate(buildPrompt(useGrounding), { grounded: useGrounding, model: genModel, maxTokens: maxTokens + (useGrounding ? GROUNDED_THINKING : 0), thinkingBudget: useGrounding ? GROUNDED_THINKING : 0, onToken: stream, onPhase: phase }); } catch (e) { out = null; lastDiag = { threw: String((e && e.message) || e).slice(0, 200) }; }
    if (out && out.streamDiag) lastDiag = out.streamDiag;
    if (useGrounding && (!out || !out.text)) {
      wasGrounded = false;
      // Count it: a few of these and groundingTripped() stops the rest of the
      // day's generations from paying this same wasted round trip.
      await bumpSurface(sql, 'ground:empty');
