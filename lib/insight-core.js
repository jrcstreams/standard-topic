...
async function gatherTrendFacts(sql, query) {
  ...
  let fromSerp = false;
  if (facts.length < TREND_RAG_MIN_DB) {
    const serp = await serpTrendNews(query, 6);
    if (serp.length) {
      fromSerp = true;
      const seen = new Set(facts.map((f) => f.url));
      for (const s of serp) if (!seen.has(s.url)) { seen.add(s.url); facts.push(s); }
    }
  }
