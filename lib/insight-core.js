...
async function generateInsight(sql, input) {
  ...
  if (type === 'news') { prompt = newsPrompt(entity); maxTokens = 900; }
    ...
    const headlines = await overviewHeadlines(sql, entity.topic, entity.group);
    prompt = overviewPrompt(entity.topic, entity.group, entity.sections, useGrounding, headlines);
