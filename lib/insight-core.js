...
async function generateInsight(sql, input, opts = {}) {
  ...
  if (type === 'shortcut' && input.builder) {
    ...
    try { out = await generate(buildPrompt(useGrounding), { grounded: useGrounding, model: genModel, maxTokens: maxTokens + (useGrounding ? GROUNDED_THINKING : 0), thinkingBudget: useGrounding ? GROUNDED_THINKING : 0, onToken: stream }); } catch (_) { out = null; }
    if (useGrounding && (!out || !out.text)) {
      ...
      try { out = await generate(buildPrompt(false), { grounded: false, model: genModel, maxTokens, onToken: stream }); } catch (_) { out = null; }
    }
    if (!out || !out.text) return { unavailable: true };
    content = out.text; sources = out.citations || [];
