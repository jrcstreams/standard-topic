...
// entity and caches it. Returns a plain object:
//   { content, sources, cached } | { unavailable:true } | { capped:true } | { error }
//
...
const shortcutsDirectory = require('../data/shortcuts-directory.json');
...
// Directory shortcuts by id — per-shortcut "item" briefs use the shortcut's
// own prompt, resolved server-side (client-supplied prompt text is never used).
const SHORTCUT_BY_ID = new Map(
  (shortcutsDirectory.shortcuts || []).map((s) => [s.id, s]));
...
function shortcutItemPrompt(item) {
  return [
    item.prompt,
    `Write 3-5 sentences of plain prose, then 3-4 key takeaways as "- " bullet lines. Accurate and specific — do not invent.`,
    `Use Google Search for current facts and cite sources.`,
  ].join('\n');
}
...
async function generateInsight(sql, input) {
  ...
  if (type === 'shortcut' && input.id) {
    // Per-shortcut "item" brief — prompt comes from the directory, by id.
    const item = SHORTCUT_BY_ID.get(String(input.id).trim());
    if (!item) return { error: 'Unknown shortcut id' };
    insight = 'item'; entity = { item }; key = item.id;
  } else if (type === 'shortcut') {
    const group = String(input.group || '').trim().toLowerCase();
    const topic = String(input.topic || '').trim();
    const lens = SHORTCUT_LENS[group];
    if (!lens || !topic) return { error: 'Missing/invalid shortcut group or topic' };
  ...
  if (hit.length) return { content: hit[0].content, sources: hit[0].sources || [], cached: true };
  ...
  else if (insight === 'item') { prompt = shortcutItemPrompt(entity.item); maxTokens = 700; }
  ...
  return { content: out.text, sources, cached: false };
}
