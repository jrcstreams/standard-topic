...
async function generateInsight(sql, input, opts = {}) {
  ...
  const NEWS_GROUND_ONDEMAND = process.env.AI_NEWS_GROUND_ONDEMAND === '1';
  const newsTierBlocks = type === 'news' && !input.internal && !NEWS_GROUND_ONDEMAND;
  const useGrounding = GROUNDING && lensGrounded && searchesSoFar < groundCeil
    && !tripped && !newsTierBlocks;
