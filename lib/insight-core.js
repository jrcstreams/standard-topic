...
// Recent real headlines from our own news feed, fed into time-sensitive overview
// prompts as primary source material (Fix 2 from the grounding plan). Skipped for
// the evergreen Learn lens (live headlines aren't relevant to fundamentals).
async function overviewHeadlines(sql, scope, group) {
  if (group === 'learn') return [];
  const topic = resolveTopic(scope);
  if (!topic) return [];
  try {
    if (topic.slug === 'home') {
      // Global: spread the newest stories across topics so "Global Headlines"
      // isn't dominated by one feed.
      return await sql.query(
        `SELECT DISTINCT ON (ns.topic_id) ns.title, ns.url
           FROM news_stories ns
          WHERE coalesce(ns.published_at, ns.fetched_at) > now() - interval '36 hours'
          ORDER BY ns.topic_id, coalesce(ns.published_at, ns.fetched_at) DESC NULLS LAST
          LIMIT 16`);
    }
    return await sql.query(
      `SELECT ns.title, ns.url FROM news_stories ns JOIN topics tp ON tp.id = ns.topic_id
        WHERE tp.slug = $1 AND coalesce(ns.published_at, ns.fetched_at) > now() - interval '96 hours'
        ORDER BY coalesce(ns.published_at, ns.fetched_at) DESC NULLS LAST LIMIT 10`, [topic.slug]);
  } catch (_) { return []; }
}
...
async function generateInsight(sql, input) {
  ...
  // 3. Prompt.
  let prompt; let maxTokens; let overviewHeads = [];
  if (type === 'news') { prompt = newsPrompt(entity); maxTokens = 900; }
    ...
    overviewHeads = await overviewHeadlines(sql, entity.topic, entity.group);
    prompt = overviewPrompt(entity.topic, entity.group, entity.sections, useGrounding, overviewHeads);
  ...
  // For overviews: if Google grounding surfaced no citation chunks, fall back to
  // the real headlines we fed as source material — they ARE what the brief drew
  // from, so the Sources list links to current, relevant articles instead of
  // being empty.
  if (type === 'shortcut' && !sources.length && overviewHeads.length) {
    sources = overviewHeads.slice(0, 8).map((h) => ({ title: h.title || '', uri: h.url || '' })).filter((s) => s.uri);
  }
  ...
  try {
    ...
    await sql.query(
      ...
      [type, key, insight, content, INSIGHT_MODEL]);
  }
