...
function builderPrompt(scope, group, grounded, headlines) {
  ...
  let headlineBlock = '';
  if (headList) {
    if (group === 'discover' || group === 'daily') headlineBlock = `\n\nCURRENT HEADLINES from our news feed (newest first), as primary source material — reference concrete stories by name where relevant${group === 'daily' ? ' (the 101 Info section stays evergreen: use these only to verify durable facts there, never as its subject)' : ''}:\n${headList}`;
