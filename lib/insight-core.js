...
async function generateInsight(sql, input) {
  ...
  if (type === 'shortcut') {
    ...
    let sections = resolveSections(topic, group);
    if (!sections) {
      // Custom / free-text term (not in the topic library). Only the Insight
      // Builders support it — they generate from the term itself + live grounding
      // (no curated shortcuts to anchor on). The legacy per-section overview path
      // still requires a known topic.
      if (!input.builder) return { error: 'Unknown topic' };
      sections = [];
    }
  ...
  if (type === 'shortcut' && input.builder) {
    ...
    try { out = await generate(p, { grounded: useGrounding, model: genModel, maxTokens: 2048 }); } catch (_) { out = null; }
