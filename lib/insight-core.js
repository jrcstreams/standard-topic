...
async function generateInsight(sql, input) {
  ...
  if (type === 'shortcut' && input.builder) {
    ...
    wasGrounded = useGrounding;
    try { out = await generate(prompt, { grounded: useGrounding, model: genModel, maxTokens: maxTokens + (useGrounding ? GROUNDED_THINKING : 0), thinkingBudget: useGrounding ? GROUNDED_THINKING : 0 }); } catch (_) { out = null; }
