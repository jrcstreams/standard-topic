...
async function generateInsight(sql, input, opts = {}) {
  ...
  if (type === 'shortcut' && input.builder) {
    ...
    let lastDiag = null;
    try { out = await generate(buildPrompt(useGrounding), { grounded: useGrounding, model: genModel, maxTokens: maxTokens + (useGrounding ? GROUNDED_THINKING : 0), thinkingBudget: useGrounding ? GROUNDED_THINKING : 0, onToken: stream }); } catch (e) { out = null; lastDiag = { threw: String((e && e.message) || e).slice(0, 200) }; }
