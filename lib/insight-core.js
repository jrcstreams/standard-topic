...
function builderPrompt(scope, group, grounded, headlines) {
  ...
  let headlineBlock = '';
  if (headList) {
    if (group === 'discover' || group === 'daily') headlineBlock = `\n\nCURRENT HEADLINES from our news feed (newest first), as primary source material — reference concrete stories by name where relevant${group === 'daily' ? '. This list is also your PREVALENCE SIGNAL: weight the briefing toward what many of these headlines cover, and keep sub-areas that appear only once or twice proportionally small' : ''}:\n${headList}`;
