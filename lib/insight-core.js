...
// Grounding is the real cost lever — NOT tokens. Google bills per SEARCH QUERY
// (one grounded call fires several) at GROUNDING_PER_SEARCH_MICROS ($35 / 1,000
// = 35 millionths), free up to GROUNDING_FREE queries/day. We DEFAULT FREE=0 so
// the spend estimate prices every search — the conservative/over-estimate side,
// which is what you want for a cost watchdog. If your tier genuinely grants a
// free grounding allowance (1,500/day on paid Tier 1) and you'd rather the
// estimate reflect it, set GEMINI_GROUNDING_FREE=1500. Note SEARCH_BUDGET (the
// point we stop issuing grounded calls) is independent of this — changing FREE
// only moves the dollar estimate, never the actual API behaviour.
...
// Learn overviews are evergreen/explanatory — no live grounding needed.
const LENS_GROUNDED = { discover: true, learn: false, analyze: true, 'topic-specific': true };
...
function overviewPrompt(scope, group, sections, grounded) {
  ...
  return grounded ? `${body}\nUse Google Search for current facts and cite sources.` : body;
}
