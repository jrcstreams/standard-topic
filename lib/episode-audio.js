...
function paceReport(mp3Path, chapters, segments, durationMs) {
  // silencedetect reports on stderr; execFileSync hands back stdout, so this
  // reads the stderr stream explicitly (the first cut read an empty string
  // and reported 0% pauses everywhere).
  const run = require('child_process').spawnSync('ffmpeg', ['-hide_banner', '-nostats', '-i', mp3Path, '-af', 'silencedetect=noise=-32dB:d=0.22', '-f', 'null', '-'], { encoding: 'utf8', maxBuffer: 64 * 1024 * 1024 });
  const log = String(run.stderr || '') + String(run.stdout || '');
