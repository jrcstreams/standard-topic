...
async function generate(prompt, { maxTokens = 320, temperature = 0.3, grounded = false, model: modelOverride, thinkingBudget = 0 } = {}) {
  ...
  const reqBody = {
    ...
    // otherwise eat the output budget and truncate the answer. BUT 2.5 models
    // decide to invoke the google_search tool DURING thinking, so grounded calls
    // need a thinking budget or no live search fires — pass thinkingBudget>0.
