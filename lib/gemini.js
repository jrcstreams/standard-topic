// Minimal Gemini client (REST, no SDK) for serverless functions.
// Used for lazy AI summaries (api/insight.js) and, later, embeddings.
// Returns null when GEMINI_API_KEY is unset so callers degrade gracefully.
//
// Model + price are env-overridable so we can retune without a deploy:
//   GEMINI_MODEL        (default gemini-2.5-flash)
//   GEMINI_IN_MICROS    cost per input token, in millionths of USD  (default 0.30)
//   GEMINI_OUT_MICROS   cost per output token, in millionths of USD (default 2.50)
// Defaults now track gemini-2.5-flash list pricing ($0.30 / $2.50 per 1M tokens):
// the whole insight pipeline is consolidated onto 2.5-flash (revamp613) — the old
// flash-lite secondary tier is retired since the user-facing surfaces already ran
// flash. Grounding (Google Search) is billed separately, per search query past the
// daily free tier (1,500/day on 2.5, effectively free at our volume); insight-core
// adds that on top of these token micros. Do NOT move to Gemini 3 without handling
// its metered grounding (5k/mo free) — see project_gemini_cost memory.

const BASE = 'https://generativelanguage.googleapis.com/v1beta';

function model() { return process.env.GEMINI_MODEL || 'gemini-2.5-flash'; }
function inMicros() { return Number(process.env.GEMINI_IN_MICROS || 0.30); }
function outMicros() { return Number(process.env.GEMINI_OUT_MICROS || 2.50); }

// Read a Gemini `?alt=sse` stream and fold it back into the SAME shape a
// one-shot generateContent response produces, so the caller can't tell the
// difference. Chunks each carry a partial candidate; text accumulates across
// them while groundingMetadata/usageMetadata/finishReason arrive late (usually
// on the final chunk), so we keep the last non-empty value of each.
//
// onToken receives ANSWER text only. With a thinking budget the model also
// streams `thought: true` parts — those are its reasoning, never shown to the
// user, and must not reach the client.
async function readSSE(res, onToken, onPhase) {
  const reader = res.body.getReader();
  const decoder = new TextDecoder();
  let buf = '';
  let text = '';
  let meta = null; let usage = null; let finish; let partCount = 0;
  let bytes = 0; let frames = 0; let raw = '';
  let sawThought = false; let sawAnswer = false;

  // Fold one parsed GenerateContentResponse chunk into the accumulators.
  const absorb = (chunk) => {
    frames += 1;
    const cand = (chunk.candidates && chunk.candidates[0]) || {};
    const parts = cand.content && cand.content.parts;
    if (Array.isArray(parts)) {
      partCount += parts.length;
      for (const p of parts) {
        // thought:true is the model's private reasoning — never show it, but DO
        // report that it's happening: a grounded call spends several seconds
        // thinking and searching before a single answer token exists, and a
        // silent spinner through that window is the thing we're fixing.
        if (!p || p.thought === true || !p.text) {
          if (p && p.thought === true && onPhase && !sawThought) { sawThought = true; try { onPhase('thinking'); } catch (_) {} }
          continue;
        }
        if (onPhase && !sawAnswer) { sawAnswer = true; try { onPhase('writing'); } catch (_) {} }
        text += p.text;
        if (onToken) { try { onToken(p.text); } catch (_) { /* a render fault must not kill the stream */ } }
      }
    }
    if (cand.groundingMetadata) meta = cand.groundingMetadata;
    if (cand.finishReason) finish = cand.finishReason;
    if (chunk.usageMetadata) usage = chunk.usageMetadata;
  };

  // Pull complete `data:` frames out of the buffer. SSE permits CRLF, LF or CR
  // line endings, so the buffer is normalized to LF before we look for the
  // blank-line frame separator — matching only "\n\n" silently consumes the
  // entire stream when the server frames with CRLF.
  const drain = (final) => {
    buf = buf.replace(/\r\n/g, '\n').replace(/\r/g, '\n');
    let nl;
    while ((nl = buf.indexOf('\n\n')) !== -1) {
      const frame = buf.slice(0, nl); buf = buf.slice(nl + 2);
      emit(frame);
    }
    if (final && buf.trim()) { emit(buf); buf = ''; }
  };
  const emit = (frame) => {
    // A frame may carry data across several `data:` lines (SSE concatenates them).
    let payload = '';
    for (const l of frame.split('\n')) if (l.startsWith('data:')) payload += l.slice(5).trim();
    if (!payload || payload === '[DONE]') return;
    let chunk; try { chunk = JSON.parse(payload); } catch (_) { return; }
    absorb(chunk);
  };

  for (;;) {
    const { value, done } = await reader.read();
    if (done) break;
    bytes += value ? value.length : 0;
    const s = decoder.decode(value, { stream: true });
    // Keep the whole body ONLY while nothing has parsed as SSE — that's the
    // case the JSON-array fallback below needs it for. Once frames are landing
    // we stop retaining it, so a long brief doesn't hold a second copy.
    if (!frames) raw += s;
    buf += s;
    drain(false);
  }
  drain(true);

  // Fallback: if nothing parsed as SSE, the body may be the plain
  // streamGenerateContent form — a JSON array of chunks — rather than an event
  // stream. Parse it wholesale so a transport surprise degrades to working
  // (non-progressive) output instead of an empty brief.
  if (!frames && raw.trim()) {
    try {
      const arr = JSON.parse(raw.trim());
      if (Array.isArray(arr)) arr.forEach(absorb);
      else if (arr && arr.candidates) absorb(arr);
    } catch (_) { /* genuinely unparseable */ }
  }

  return { text, meta, usage, finish, partCount, _diag: { bytes, frames, head: raw.slice(0, 200) } };
}

// Generate text. With { grounded:true } the model uses Google Search at
// generation time (live-accurate + returns citations). opts.model overrides
// the model. Returns { text, citations:[{title,uri}], inTok, outTok, micros }
// or null if no key.
//
// Pass opts.onToken to STREAM: the call switches to streamGenerateContent and
// onToken(delta) fires as answer text arrives, so a caller can paint a brief
// progressively instead of blocking ~10s on the whole thing. The return value
// is identical either way — streaming is purely additive, and every existing
// caller (crons, background heal) is untouched.
// revamp1246 — retry the failures that are worth retrying, and only those.
//
// Every non-200 used to throw straight to the caller, so a single 429 during a
// traffic spike became a user-visible "couldn't load this insight" — even though
// the next second would have succeeded. Retries cover 429 (rate limited) and 5xx
// (Google's side); 400/403/404 are OUR bug or key problem and retrying them just
// spends latency before the same error.
//
// Backoff is exponential with jitter. The jitter matters more than the backoff:
// N visitors hitting the same rate limit retry in lockstep without it, which is
// how a spike becomes a thundering herd. Retry-After is honoured when Google
// sends it — it knows better than our schedule does.
//
// Total added latency in the worst case is ~3.5s against a 60s function budget,
// so a retried call still returns well inside the request.
const RETRY_STATUS = new Set([408, 429, 500, 502, 503, 504]);
const MAX_ATTEMPTS = parseInt(process.env.GEMINI_MAX_ATTEMPTS || '3', 10);

function retryDelayMs(attempt, res) {
  const hdr = res && res.headers && res.headers.get && res.headers.get('retry-after');
  if (hdr) {
    const secs = Number(hdr);
    // Cap a server-supplied wait: Gemini occasionally returns a long one, and
    // blocking a request for 30s is worse for the reader than failing fast.
    if (Number.isFinite(secs) && secs > 0) return Math.min(secs * 1000, 4000);
  }
  const base = 400 * Math.pow(2, attempt);          // 400, 800, 1600
  return Math.round(base * (0.5 + Math.random()));  // ±50% jitter
}

async function fetchWithRetry(url, init) {
  let last = null;
  for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
    let res = null;
    try {
      res = await fetch(url, init);
    } catch (err) {
      // Network-level failure (socket reset, DNS). Retry on the same terms as a
      // 5xx — it is transport, not a rejected request.
      last = err;
      if (attempt === MAX_ATTEMPTS - 1) throw err;
      await new Promise((r) => setTimeout(r, retryDelayMs(attempt, null)));
      continue;
    }
    if (res.ok || !RETRY_STATUS.has(res.status)) return res;
    last = res;
    if (attempt === MAX_ATTEMPTS - 1) return res;   // caller reads + throws
    // The body must be drained before the socket can be reused.
    try { await res.text(); } catch (_) { /* nothing to drain */ }
    const wait = retryDelayMs(attempt, res);
    console.warn(`[gemini] ${res.status} — retry ${attempt + 1}/${MAX_ATTEMPTS - 1} in ${wait}ms`);
    await new Promise((r) => setTimeout(r, wait));
  }
  return last;
}

async function generate(prompt, { maxTokens = 320, temperature = 0.3, grounded = false, model: modelOverride, thinkingBudget = 0, onToken = null, onPhase = null, responseSchema = null } = {}) {
  const key = process.env.GEMINI_API_KEY;
  if (!key) return null;
  const m = modelOverride || model();
  const method = onToken ? 'streamGenerateContent' : 'generateContent';
  const qs = onToken ? `?alt=sse&key=${encodeURIComponent(key)}` : `?key=${encodeURIComponent(key)}`;
  const url = `${BASE}/models/${encodeURIComponent(m)}:${method}${qs}`;
  const reqBody = {
    contents: [{ parts: [{ text: prompt }] }],
    // 2.5 models decide to invoke the google_search tool DURING their thinking
    // phase, so grounded calls need thinkingBudget>0 or no live search fires and
    // the answer comes from stale training data. IMPORTANT: thinking tokens count
    // against maxOutputTokens, so callers must pass maxTokens = answerBudget +
    // thinkingBudget — otherwise thinking eats the budget and the answer truncates
    // mid-sentence (the trend briefs cutting off at "Mexico 3"). See insight-core.
    generationConfig: {
      maxOutputTokens: maxTokens, temperature,
      // includeThoughts only when STREAMING: it's what lets us tell the user
      // "searching sources" during the long pre-answer phase. The thought text
      // itself is filtered out in readSSE and never leaves the server.
      thinkingConfig: onToken && thinkingBudget ? { thinkingBudget, includeThoughts: true } : { thinkingBudget },
    },
  };
  // Structured output (episode desk/writer/QA). When a schema is supplied the
  // model is constrained to emit JSON matching it, so the caller can JSON.parse
  // without the "```json fence, sometimes" dance. Mutually exclusive with
  // grounding in the API, which is fine: every structured caller here works from
  // already-grounded briefs and never needs live search.
  if (responseSchema) {
    reqBody.generationConfig.responseMimeType = 'application/json';
    reqBody.generationConfig.responseSchema = responseSchema;
  }
  if (grounded) reqBody.tools = [{ google_search: {} }];
  const res = await fetchWithRetry(url, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(reqBody),
  });
  if (!res.ok) {
    const detail = await res.text().catch(() => '');
    throw new Error(`Gemini ${res.status} ${detail.slice(0, 240)}`);
  }
  let cand; let parts; let text; let u; let streamDiag = null;
  if (onToken) {
    const s = await readSSE(res, onToken, onPhase);
    cand = { groundingMetadata: s.meta, finishReason: s.finish };
    parts = new Array(s.partCount);
    text = s.text;
    u = s.usage || {};
    streamDiag = s._diag;
  } else {
    const data = await res.json();
    cand = (data && data.candidates && data.candidates[0]) || {};
    parts = cand.content && cand.content.parts;
    text = Array.isArray(parts) ? parts.map(p => p.text || '').join('') : '';
    u = (data && data.usageMetadata) || {};
  }
  // Strip inline grounding citation markers some models emit (e.g. [cite_start]).
  text = text.replace(/\[cite[^\]]*\]/gi, '').replace(/[ \t]+\n/g, '\n').trim();
  const meta = cand.groundingMetadata || null;
  const chunks = (cand.groundingMetadata && cand.groundingMetadata.groundingChunks) || [];
  // revamp1191 — groundingSupports is the part that says WHICH claim each source
  // supports: every entry pairs a segment of the answer with the chunk indices
  // backing it. We were keeping only the flat chunk list, which is why the
  // client had to guess per-story attribution from word overlap.
  //
  // Segment offsets are indices into the ORIGINAL response text, and the text is
  // rewritten downstream (cite markers stripped, the SUMMARY line lifted out,
  // trimmed), so offsets don't survive. The quoted segment text does: store it
  // and let the reader match a source to a story by containment, which needs no
  // heuristic at all.
  const supports = (meta && Array.isArray(meta.groundingSupports)) ? meta.groundingSupports : [];
  const quotesByChunk = new Map();
  for (const sup of supports) {
    const seg = (sup && sup.segment) || null;
    const quote = String((seg && seg.text) || '').trim();
    if (!quote || quote.length < 24) continue;      // too short to identify a claim
    for (const idx of (sup.groundingChunkIndices || [])) {
      if (!quotesByChunk.has(idx)) quotesByChunk.set(idx, []);
      const arr = quotesByChunk.get(idx);
      if (arr.length < 6 && !arr.includes(quote)) arr.push(quote);
    }
  }
  const seen = new Set();
  const citations = chunks
    .map((c, i) => ({ w: c && c.web, i }))
    .filter((c) => c.w)
    .map(({ w, i }) => {
      const q = quotesByChunk.get(i) || [];
      const out = { title: w.title || '', uri: w.uri || '' };
      if (q.length) out.quotes = q;
      return out;
    })
    .filter(c => c.uri && !seen.has(c.uri) && seen.add(c.uri))
    // Multi-section group overviews cite many distinct sources; 8 keeps the
    // Sources list substantial without dumping every chunk.
    .slice(0, 8);
  const inTok = u.promptTokenCount || 0;
  const outTok = u.candidatesTokenCount || 0;
  // Thinking/reasoning tokens are billed at the OUTPUT rate but reported
  // separately by the API. Fold them into the cost estimate so turning on a
  // thinking budget (for reliable grounding) doesn't silently under-report spend.
  const thoughtTok = u.thoughtsTokenCount || 0;
  const micros = Math.round(inTok * inMicros() + (outTok + thoughtTok) * outMicros());
  // Google bills grounding per SEARCH QUERY, and one grounded call can fire
  // several (webSearchQueries lists them). This is the true billable unit and
  // what counts against the daily free tier — callers meter on it, not on calls.
  const searches = (meta && Array.isArray(meta.webSearchQueries)) ? meta.webSearchQueries.length : 0;
  return { text, citations, inTok, outTok, thoughtTok, micros, searches, meta, parts: Array.isArray(parts) ? parts.length : 0, finish: cand.finishReason, streamDiag };
}

// Batch-embed texts (up to 100 per call) with Gemini gemini-embedding-001.
// Returns an array of `dim`-dim vectors (one per input), or null if no key.
// Default 256 dims (Matryoshka truncation) — 3× smaller than 768 with minor recall
// loss, to keep the pgvector column small on the free Neon tier (#storage-trim).
// MUST match the news_stories.embedding column type. Override with GEMINI_EMBED_DIM.
async function embed(texts) {
  const key = process.env.GEMINI_API_KEY;
  if (!key || !texts || !texts.length) return null;
  const m = process.env.GEMINI_EMBED_MODEL || 'gemini-embedding-001';
  const dim = parseInt(process.env.GEMINI_EMBED_DIM || '256', 10);
  const url = `${BASE}/models/${m}:batchEmbedContents?key=${encodeURIComponent(key)}`;
  const body = {
    requests: texts.map(t => ({
      model: `models/${m}`,
      content: { parts: [{ text: String(t || '').slice(0, 8000) }] },
      outputDimensionality: dim,
    })),
  };
  const res = await fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
  if (!res.ok) {
    const detail = await res.text().catch(() => '');
    throw new Error(`Gemini embed ${res.status} ${detail.slice(0, 200)}`);
  }
  const data = await res.json();
  return (data.embeddings || []).map(e => e.values);
}

// Embed a single query string → vector, or null.
async function embedQuery(text) {
  const v = await embed([text]);
  return v && v[0] ? v[0] : null;
}

// Render a JS number[] as a pgvector literal: [0.1,0.2,…]
function toVector(values) { return '[' + values.join(',') + ']'; }

module.exports = { generate, model, embed, embedQuery, toVector };
