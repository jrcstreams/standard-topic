...
// NB: 'analyze' (Analysis) was merged into 'topic-specific' (Deep Dive) — its
// builder is no longer generated/refreshed by the cron.
const AI_LENSES = ['discover', 'learn', 'topic-specific'];
const GROUP_LABELS = {
...
// Mirrors groupShortcuts() fallback regexes exactly. (The old ANALYZE_RE bucket
// was dropped when Analysis merged into Deep Dive — those names now fall through
// to 'topic-specific'.)
...
function resolveTopic(scope) {
  ...
  // revamp1446: `scope` is the topic's coverage when the display name is
  // shorter than what it covers ("Business" still curates finance).
  return t ? { slug: t.slug, name: t.name, scope: t.scope || '' } : null;
}
