...
async function bulkInsert(sql, table, columns, rows, opts = {}) {
  ...
  // opts.returnIds → array of inserted ids (rows skipped by ON CONFLICT return none);
  // default stays the count so existing callers are untouched.
  return opts.returnIds ? out.map((r) => r.id) : out.length;
}
