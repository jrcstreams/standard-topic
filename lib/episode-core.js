...
async function runFactCheck(fmt, { script, dateLabel }, { model = textModel(), thinking = 2048 } = {}) {
  ...
  const prompt = [
    ...
    ``,
    `HOW TO CORRECT: change the fewest words that make the sentence true. Do not add stories, do not reorder, do not drop a sentence unless it is false and unfixable, keep each segment within ten percent of its current length, keep the spoken style (numbers spelled out as said aloud). Segments you did not change come back verbatim. Set "written" to the segment's corrected written item in plain third person if the segment had one; otherwise leave it empty.`,
    ``,
    body,
  ].join('\n');
  const budget = Math.max(9000, Math.round(fmt.minutes * WORDS_PER_MIN * 3));
  const out = await generate(prompt, { model, grounded: true, maxTokens: budget + thinking, thinkingBudget: thinking, temperature: 0.2, responseSchema: SCRIPT_SCHEMA });
  const checked = normalizeScript(parseJSON(out, 'factcheck'));
  ...
  return { script: checked, usage: out, changes };
}
