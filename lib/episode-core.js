...
// revamp1433: the label is the EDITION's date. Taken off waveStart it read
// "Wednesday, September 16" on the September 17 evening edition, because that
// wave resolves backwards until the release hour has passed.
function todayLabel(now = new Date()) {
  const ed = currentEdition(now);
...
`1. DE-DUPLICATE ACROSS FAMILIES. The same story appears in several topics (an AI chip export rule shows up under Technology, Business and World). Merge those into ONE segment and list every briefing slug it draws on in brief_refs. Never let one story occupy two segments.`,
...
// revamp1436 — A CORRECTION MAY NOT REVERSE A CLAIM UNSUPPORTED.
//
// Tonight's episode opened by saying the Federal Reserve "lowered interest
// rates", while its own storyboard said "Fed Hikes Rates" and the briefing it
// was written from said hikes. The fact checker did that: it rewrote the lead
// and inverted the verb. A checker that can silently flip the direction of a
// story is worse than no checker, because it is trusted.
//
// So a correction that turns a rise into a fall — or an approval into a
// rejection — has to be supported by something. It is kept only when the
// dossier states the new direction in its own words. Otherwise the original
// sentence stands and the flip is logged.
...
async function runFactCheck(fmt, { script, dateLabel, card: ctxCard }, { model = textModel(), thinking = 2048 } = {}) {
  ...
  const fixPrompt = [
    ...
    ``,
    `RULES: the dossier is the source of truth where it speaks; where it says "confirmed" or is silent, the text stays verbatim. Change the fewest words that make a sentence true. DIRECTION IS NOT A DETAIL: never turn a rise into a fall, a raise into a cut, an approval into a rejection or a win into a loss unless the dossier says so in those words. A dossier line that gives a LEVEL ("the target range is 4.00%-4.25%") says nothing about direction — if it does not state the direction, the script's direction stands. When a figure and a direction disagree, correct the figure and leave the direction. A level the dossier shows as already crossed is stated as its current state ("above one hundred dollars a barrel since last week"), never as a threshold ahead. Do not add stories, do not reorder, do not drop a sentence unless it is false and unfixable, keep each segment within ten percent of its length. SPOKEN keeps the spoken style (numbers said aloud); WRITTEN and the card use numerals.`,
    ``,
  ...
  const segments = segs.map((s, i) => {
    ...
    const n = { ...s };
    const label = `${s.beat}${s.chapter ? ` (${s.chapter})` : ''}`;
    const flip = spoken ? directionFlip(String(s.text || ''), spoken, dossier) : null;
    if (flip) {
      // Unsupported reversal: keep what the writer wrote, which agreed with the
      // briefing it came from, and say so loudly enough to notice.
      console.warn(`[factcheck] REJECTED a ${flip} reversal in ${label} — the dossier does not support it; keeping the original`);
      reversals.push(`${label}: ${flip}`);
      return { ...s };
    }
    if (spoken && spoken !== String(s.text || '').trim()) { n.text = spoken; n.paragraphs = spoken.split(/\n{2,}/).map((p) => p.trim()).filter(Boolean); changes.push(label); }
    if (written && written !== String(s.written || '').trim()) { n.written = written; if (!changes.includes(label)) changes.push(`${s.beat} written`); }
    return n;
  });
  ...
  if (reversals.length) usage.reversals = reversals;
