...
function editionLabel(ed) { return ed === 'evening' ? 'Global AI Evening Briefing' : 'Global AI Morning Briefing'; }
...
yesterday ? `9. CONTINUITY. Yesterday's rundown led with: "${yesterday}". If the same story leads again, the angle must frame it as a development ("what changed overnight"), never as if it were breaking now.` : ``,
``,
...
const SCRIPT_SCHEMA = {
  ...
  properties: {
    ...
    // Three scannable lines for the briefing card ("In Focus"), max 12 words
    // each, one per story segment in order of consequence.
    in_focus: { type: 'ARRAY', items: { type: 'STRING' } },
  },
  required: ['segments', 'in_focus'],
};
...
function writerPrompt(fmt, { storyboard, selected, dateLabel, edition = 'morning' }) {
  ...
  const nextTime = isEve ? 'tomorrow morning' : 'this evening';
  const beats = [];
...
`in_focus: exactly THREE lines, max 12 words each, one per story segment in order of consequence (the lead first). They are the card headline for the day, so each must name a concrete development, not a theme.`,
``,
`=== THE RUNDOWN ===`,
...
function renderBriefingText(script, storyboard, { chapters = [] } = {}) {
  ...
  const lines = [];
  lines.push(`SUMMARY: ${String(storyboard.teaser || '').trim()}`);
  lines.push('', '## Overview');
  ...
  let focus = (script.in_focus || []).map((l) => String(l).trim()).filter(Boolean).slice(0, 3);
  if (focus.length < 3) {
    const fromTitles = segs.filter((x) => ['lead', 'developing', 'deep_dive', 'hit'].includes(x.beat)).map((x) => String(x.chapter || '').trim()).filter(Boolean);
    focus = focus.concat(fromTitles.filter((t) => !focus.includes(t))).slice(0, 3);
  }
  for (const l of focus) lines.push(`- ${l}`);
...
module.exports = {
  ...
  speak, speakChecked, speechChunks, voiceScript, showNotes, scriptToText, renderBriefingText, chapterLabels,
