...
const EDITION_HOURS_ET = { morning: 5, evening: 17 };
function editionFor(now = new Date()) {
  const forced = process.env.EPISODE_EDITION;
  if (forced === 'morning' || forced === 'evening') return forced;
  // revamp1403: the evening edition was scrapped (one briefing a day); a run
  // after 5pm ET is still the morning edition, republished.
  return 'morning';
  const h = etParts(now).h;
  return h >= EDITION_HOURS_ET.evening || h < EDITION_HOURS_ET.morning ? 'evening' : 'morning';
}
function editionLabel(ed) { return ed === 'evening' ? 'Global AI Evening Briefing' : 'Morning AI Briefing'; }
...
async function gatherBriefs(sql, { since = null } = {}) {
  const { byName, bySlug } = topicIndex();
  const rows = await sql.query(
    `SELECT entity_key, content, summary, sources, created_at
       FROM ai_insights
  ...
  for (const r of rows) {
    ...
    out.push({
      ...
      createdAt: created,
      fresh: !!(created && cutoff && created.getTime() >= cutoff),
    });
  }
  return out;
}
...
async function runFactCheck(fmt, { script, dateLabel, card: ctxCard }, { model = textModel(), thinking = 2048 } = {}) {
  ...
  const card = ctxCard || {};
  const cardText = [card.headline ? `HEADLINE: ${card.headline}` : '', card.teaser ? `TEASER: ${card.teaser}` : ''].filter(Boolean).join('\n');
  const body = segs.map((s, i) => `### segment ${i} (${s.beat}${s.chapter ? ` — ${s.chapter}` : ''})\n${String(s.text || '')}`).join('\n\n')
    + (cardText ? `\n\n### segment ${segs.length} (card — the site's headline and teaser for this episode)\n${cardText}` : '');
  const prompt = [
    `You are the FACT CHECKER for Standard Topic's daily news podcast, ${dateLabel}. Below is a finished script. Use Google Search to verify it against reporting from the LAST 24 HOURS, then return the SAME segments, corrected. Search for each story's current state before you judge it.`,
    ``,
    `WHAT TO CHECK, in every story segment (${[...STORY].join(', ')}):`,
    `1. FIGURES AND THRESHOLDS. Every price, index level, count, percentage or amount. A level described as "toward", "approaching", "nearing", "on the verge of" or "expected to reach" a threshold that current reporting shows already crossed is WRONG — replace it with the current state and, where the sources give it, when it crossed ("above one hundred dollars a barrel since last week"). State the latest figure.`,
    `2. STATUS. "Expected", "planned", "set to", "is considering" — if it has already happened, say so with the outcome. If it has been cancelled or reversed, say so.`,
    `3. TIME. "today", "this morning", "overnight", "just" — only if the event is dated today in current reporting; otherwise remove the word or give the real date.`,
    `4. NAMES, ROLES, PLACES — correct any that current reporting contradicts.`,
    ``,
    `HOW TO CORRECT: change the fewest words that make the sentence true. Do not add stories, do not reorder, do not drop a sentence unless it is false and unfixable, keep each segment within ten percent of its current length, keep the spoken style (numbers spelled out as said aloud). Segments you did not change come back verbatim. The card segment keeps its two labelled lines ("HEADLINE: …" and "TEASER: …"), corrected the same way, written for the page (numerals are fine there).`,
    ``,
    `OUTPUT: JSON only, no prose before or after: {"segments":[{"index":0,"text":"…"},{"index":1,"text":"…"}, …]} — one entry per segment above, in order, "text" being the segment's full corrected text.`,
    ``,
    body,
  ].join('\n');
  const budget = Math.max(9000, Math.round(fmt.minutes * WORDS_PER_MIN * 3));
  const out = await generate(prompt, { model, grounded: true, maxTokens: budget + thinking, thinkingBudget: thinking, temperature: 0.2 });
  const parsed = parseJSON(out, 'factcheck');
  const byIndex = new Map((parsed.segments || []).map((x) => [Number(x.index), String(x.text || '')]));
  const checked = { ...script, segments: segs.map((s, i) => (byIndex.has(i) && byIndex.get(i).trim()) ? { ...s, text: byIndex.get(i) } : { ...s }) };
  // the card's lines, if they came back
  let cardOut = null;
  if (cardText && byIndex.has(segs.length)) {
    const t = byIndex.get(segs.length);
    const h = (t.match(/HEADLINE:\s*(.+)/) || [])[1]; const te = (t.match(/TEASER:\s*([\s\S]+)/) || [])[1];
    cardOut = { headline: h ? h.trim() : null, teaser: te ? te.trim() : null };
  }
  // what changed, for the log
  const changes = [];
  (checked.segments || []).forEach((cs, i) => { const ws = segs[i]; if (ws && cs && String(cs.text || '').trim() && String(cs.text).trim() !== String(ws.text || '').trim()) changes.push(`${ws.beat}${ws.chapter ? ` (${ws.chapter})` : ''}`); });
  if (cardOut) { if (cardOut.headline && cardOut.headline !== card.headline) changes.push('headline'); if (cardOut.teaser && cardOut.teaser !== card.teaser) changes.push('teaser'); }
  return { script: checked, usage: out, changes, card: cardOut };
}
